...
export default function PostPage() {
    ...
    return (
      <div className="min-h-screen bg-hearten-bg flex items-center justify-center">
        <div className="text-center space-y-3">
          ...
          <button onClick={() => router.push('/')} className="px-4 py-2 rounded-lg bg-hearten-rose text-white text-sm">
            返去首頁
          </button>
        </div>
      </div>
  ...
  const handleSubmitComment = () => {
    ...
    const comment = createComment(post.id, replyText, user.username);
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span onClick={(e) => { e.stopPropagation(); router.push(`/user/${encodeURIComponent(post.anonymous)}`); }}
            ...
            <div className="flex items-center gap-2">
                ...
                {FONT_SIZES.map((f) => (
                  <button key={f.key} onClick={() => setFontSize(f.key)}
                    className={`px-2 py-1 text-xs transition-colors ${
                      fontSize === f.key ? 'bg-hearten-rose text-white' : 'text-hearten-muted hover:text-hearten-text hover:bg-hearten-card'
                    }`}>
                    {f.label}
                  </button>
                ))}
              ...
              <button onClick={() => setAuthorOnly(!authorOnly)}
                className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs transition-colors ${
                  authorOnly ? 'bg-hearten-amber/20 text-hearten-amber' : 'text-hearten-muted hover:text-hearten-text hover:bg-hearten-card'
                }`}>
        ...
        {user ? (
                ...
                {MOODS.map((m) => (
                  <button key={m.emoji} onClick={() => setMood(mood === m.label ? null : m.label)}
                    className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs transition-all ${
                      mood === m.label ? 'bg-hearten-rose/20 text-hearten-rose border border-hearten-rose/40'
          ...
          <div className="bg-hearten-card border border-hearten-border rounded-xl p-6 mb-4 text-center">
            ...
            <button onClick={() => window.dispatchEvent(new Event('hearten:open-login'))}
              className="px-4 py-2 rounded-lg bg-hearten-rose hover:bg-hearten-rose-light text-white text-sm font-medium transition-colors">
              登入 / 註冊
            </button>
          </div>
        )}
        ...
        <div className="space-y-0">
          {displayedComments.length === 0 ? (
            ...
            displayedComments.map((comment) => (
              <CommentItem key={comment.id} comment={comment} postId={post.id} onCommentAdded={refreshComments} />
            ))
          )}
        </div>
      </div>
...
function CommentItem({ comment, postId, onCommentAdded, depth = 0 }: {
  ...
  const handleReply = () => {
    ...
    const c = createComment(postId, replyInput, user.username);
    addUserComment(c);
    setReplyInput('');
    setShowReply(false);
    onCommentAdded();
