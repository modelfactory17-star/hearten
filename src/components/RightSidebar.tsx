...
export default function RightSidebar() {
  return (
    <aside className="w-[280px] shrink-0 border-l border-hearten-border h-[calc(100vh-56px)] sticky top-14 overflow-y-auto px-4 py-5 max-[1100px]:hidden">
      ...
      {/* 熱門話題 */}
      <div className="mb-7">
        <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.06em] text-hearten-muted mb-[14px] pl-0.5">
        ...
        <div className="flex flex-col gap-[2px]">
          {hotTopics.map((topic) => (
            ...
            >
              ...
              <span className="flex-1 text-[13.5px] text-hearten-muted whitespace-nowrap overflow-hidden text-ellipsis group-hover:text-hearten-text">
                {topic.text}
              </span>
              <span className="text-[11.5px] text-hearten-dim flex-shrink-0">{topic.num}</span>
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="flex flex-col gap-[2px]">
          {newMembers.map((member) => (
            ...
            >
              ...
              <span className="flex-1 text-[13.5px] text-hearten-muted whitespace-nowrap overflow-hidden text-ellipsis">
                {member.text}
              </span>
              <span className="text-[11.5px] text-hearten-dim flex-shrink-0">新</span>
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="flex flex-col gap-[2px]">
          {activeUsers.map((user) => (
            ...
            >
              ...
              <span className="text-[11.5px] text-hearten-dim flex-shrink-0">{user.num}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Footer */}
      <div className="pt-[18px] border-t border-hearten-border text-[11.5px] text-hearten-dim leading-[1.9]">
        <p>© 2024 Hearten</p>
